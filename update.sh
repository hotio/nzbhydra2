...
EOF
    ...
    version=$(curl -u "${GITHUB_ACTOR}:${GITHUB_TOKEN}" -fsSL "https://api.github.com/repos/theotherp/nzbhydra2/releases/latest" | jq -r .tag_name | sed s/v//g)
    [[ -z ${version} ]] && exit 1
    sed -i "s/{NZBHYDRA2_VERSION=[^}]*}/{NZBHYDRA2_VERSION=${version}}/g" .drone.yml
